...
cv::CascadeClassifier  H_cascader;
string Hfile = "/home/solal/Desktop/H.png";  //H图片的路径
...
void process_image() {
    ...
    if(!H_cascader.load(Hfile))
    {
        printf("could not load Hfile...\n");
    }
...
*/    auto filtered = cv::Mat();
    cv::Mat change = cv::imread("/home/solal/Desktop/map.jpg");
    ...
    cv::imshow("close_opened0", close_opened);

    // 提取并画出圆形边缘
    std::vector<cv::Rect> faces;
    H_cascader.detectMultiScale(close_opened, faces, 1.1, 1, 0);
    for(size_t t=0;t<faces.size();t++){
        rectangle(close_opened, faces[t], cv::Scalar(255,0,0),2,8,0);
    }
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "H_detect_node");
    ros::NodeHandle n;
    process_image();
